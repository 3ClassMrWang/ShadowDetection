...
void Comparator::save() const {
	...
	//写入文件到result目录
	const string statsSave = videoFolder.statsSavePath();
	ofstream o(statsSave.c_str(), ios::app | ios::out);
	if (o.is_open()) {
		o << "cm: " << tp << ' ' << fp << ' ' << fn << ' ' << tn << ' ' << nbShadowErrors;
		o.close();
	} else {
		throw string("Unable to open the file : ") + statsSave;
	}
}
